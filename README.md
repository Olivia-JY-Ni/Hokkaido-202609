...
The map-first planner starts with an empty `我的地区` list. A user searches live Google Maps predictions, previews a selected Place result with its official identity and coordinates, and explicitly adds it either to the trip as a ⭐ area or to the Candidate library. Google-created Candidates are browser-local records with editable autofill for name, address, coordinates, category, municipality, region, business status, rating, phone, website, and opening hours; they never mutate the 77-record research canonical. The verified 20-area catalog remains a read-only fallback if the live service is unavailable. New areas contain zero Candidates. Candidates are assigned only through explicit user actions, remain visible in the complete library, and can belong to at most one user-created area. Each Candidate may have a browser-local visit date and time shown in both the complete library and its area's expanded list. Selecting an area zooms the map and reveals its type-colored Candidate markers. Area order, dates, notes, membership, custom Candidates, and Candidate visit times remain browser-local preferences.
