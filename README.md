...
When two or more user areas exist, the browser requests ordered driving legs from the Maps JavaScript Routes library. It displays distance and estimated duration between consecutive areas and draws the returned route path. This is informational only: it does not rank Candidates, reorder areas, optimize the route, or mutate `data/routes.json`. Both Places API (New) and Routes API must be enabled for the restricted browser key; missing services degrade to the verified catalog, straight itinerary connectors, and the canonical research details.
