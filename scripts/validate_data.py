...
def validate(external_master: Path | None = None) -> list[str]:
    ...
    area_rows = area_catalog.get("places", [])
    place_ids = [row.get("place_id") for row in area_rows]
    if len(area_rows) != 20 or len(set(place_ids)) != len(place_ids) or None in place_ids:
        errors.append("Google Maps area search catalog must contain 20 unique place_id values")
    if area_catalog.get("provider") != "Google Maps":
        errors.append("area search catalog provider must be Google Maps")
    for row in area_rows:
        place_id = row.get("place_id", "<missing>")
        if "candidate_ids" in row:
            errors.append(f"{place_id}: search catalog must not contain default Candidate assignments")
        if row.get("place_type") != "city_or_area" or not row.get("name_zh") or not row.get("formatted_address"):
            errors.append(f"{place_id}: incomplete city/area search entity")
        if not isinstance(row.get("lat"), (int, float)) or not -90 <= row["lat"] <= 90:
            errors.append(f"{place_id}: invalid Google Maps latitude")
        if not isinstance(row.get("lon"), (int, float)) or not -180 <= row["lon"] <= 180:
            errors.append(f"{place_id}: invalid Google Maps longitude")
